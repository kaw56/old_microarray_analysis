...
opts_chunk$set(echo = FALSE, fig.path = 'figure/', dev = 'png', fig.align='center', fig.show='hold')
read_chunk('1_load.R')
@
...
@
\begin{figure}[h!]
    \caption{Mitochondrial gene expression: circadian, significant change (p < 0.05)}
...
@
%%\begin{figure}[h!]
%%    \caption{Mitochondrial gene expression: tidal, significant change (p < 0.05)}
